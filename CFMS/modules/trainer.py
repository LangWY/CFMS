...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (images_id, images, reports_ids, reports_masks, labels) in enumerate(self.train_dataloader):
            ...
            output, img_con_ls, txt_con_ls, img_cls, txt_cls = self.model(images, reports_ids, labels=labels,
                                                                          mode='train')
            img_bce_ls = self.bce_loss(img_cls, labels)
            txt_bce_ls = self.bce_loss(txt_cls, labels)
            ce_ls = self.criterion(output, reports_ids, reports_masks)  # ce_loss是交叉熵损失
            ...
            loss = ce_ls + self.img_con_loss_weight * img_con_ls + self.txt_con_loss_weight * txt_con_ls \
                   + self.img_bce_loss_weight * img_bce_ls + self.txt_bce_loss_weight * txt_bce_ls  # 总的loss
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()
            if batch_idx % self.args.log_period == 0:
                self.logger.info(
                    '[{}/{}] Step: {}/{}, CE Ls: {:.5f}, CON Ls1: {:.5f}, CON Ls2: {:.5f}, BCE Ls1: {:.5f}, BCE Ls2: {:.5f}.'
        ...
        log = {'ce_loss': ce_loss / len(self.train_dataloader), 'img_con': img_con_loss / len(self.train_dataloader),
               ...
               'img_bce_loss': img_bce_loss / len(self.train_dataloader),
               'txt_bce_loss': txt_bce_loss / len(self.train_dataloader)}
